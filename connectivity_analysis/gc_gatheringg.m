...
methods (Access = public)
    ...
    function obj = gc_gatheringg(varargin)
    ...
    file_path = "/Users/jannisborn/Downloads/";
...
%         for trial = 1:min(registration.info.trials_obj,length(metastats.beh))
        for trial = 1:length(metastats.beh)
            ...
            try 
                ...
                toc
                data = reshape(trial_data.dFF, [size(trial_data.dFF,1)*...
                    size(trial_data.dFF,2), size(trial_data.dFF,3)]);
                %data = trial_data.dFF;toc;
                %if any(isnan(data))
